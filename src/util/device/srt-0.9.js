 ...
 * This object is used to sktdefine and control firing of events for
    ...
    objects: {
        ...
        navigator: {
            children: {
                ...
                //[20120903][chisu]add preferences
                preferences:{
...
module.exports = function(success, fail, service, action, args) {
  try {
    var callbackId = service + srt.callbackId++;
    if (success || fail) {
        srt.callbacks[callbackId] = {success:success, fail:fail};
    }
    ...
    if (r.length > 0) {
        ...
        if (v.status === srt.callbackStatus.OK) {
        ...
        // If no result
        else if (v.status === srt.callbackStatus.NO_RESULT) {
            // Clear callback if not expecting any more results
            if (!v.keepCallback) {
                delete srt.callbacks[callbackId];
            }
...
module.exports = {
    ...
    initialize:function() {
        ...
        setTimeout(function() {
            if (srt.UsePolling) {
                ...
                srt.UsePolling = isPolling;
                if (isPolling == "true") {
                    srt.UsePolling = true;
                    polling();
                } else {
                    srt.UsePolling = false;
                    callback();
                }
            }
        }, 1);
        ...
        var backButtonChannel = srt.addDocumentEventHandler('backbutton', {
        ...
        srt.addDocumentEventHandler('menubutton');
        srt.addDocumentEventHandler('searchbutton');

        // Figure out if we need to shim-in localStorage and WebSQL
        // support from the native side.
        var storage = sktrequire('srt/plugin/android/storage');
        ...
        channel.join(function() {
            prompt("", "gap_init:");
        }, [channel.onSKTRuntimeReady]);
    },
    objects: {
        srt: {
            children: {
                JSCallback:{
                    path:"srt/plugin/android/callback"
                },
                JSCallbackPolling:{
                    path:"srt/plugin/android/polling"
                }
            }
        },
        navigator: {
            children: {
                app:{
                    path: "srt/plugin/android/app"
                }
            }
        },
        device:{
            path: "srt/plugin/android/device"
        },
    ...
    merges: {
        navigator: {
            children: {
                notification: {
                    path: 'srt/plugin/android/notification'
                }
            }
        }
    }
...
sktdefine("srt/plugin/applauncher", function(sktrequire, exports, module) {
	...
	var applauncher = {
		 ...
		 * @param object appArg argment
		 * @param String appAction android action
		 */
		launchApplication: function(successCallback, errorCallback, appURI , appArg, appAction) {
			// successCallback required
			if (typeof successCallback !== "function" ) {
			...
			else if(appArg != null && typeof appArg != "object"){
				throw "4rd argument must be object Type.";
			}

			exec(successCallback, errorCallback, "AppLauncher", "launchApplication", [appURI,appArg,appAction]);
		}	
	};
...
Capture.prototype.captureImage = function(successCallback, errorCallback, options){
	// successCallback required
	if (typeof successCallback !== "function" ) {
		throw "captureImage must be called with at least a success callback function as first parameter.";
	}  
	// successCallback required
	else if (successCallback === null ) {
		throw "captureImage must be called with at least a success callback function as first parameter.";
...
//[20120719][chisu]phonegap capture is not used 
//file: lib/common/plugin/capture.js
//sktdefine("srt/plugin/capture", function(sktrequire, exports, module) {
//	var exec = sktrequire('srt/exec'),
//	    MediaFile = sktrequire('srt/plugin/MediaFile');
//
//	/**
//	 * Launches a capture of different types.
//	 *
...
sktdefine("srt/plugin/preferences", function(sktrequire, exports, module) {
	/**
	 * This class provides access to preferences data.
	 * @constructor
	 */
	var utils = sktrequire("srt/utils"),
	exec = sktrequire("srt/exec");
	...
	var preferences = {
		...
		getItem: function(key){
			...
			return exec(null, null, "Preferences", "getItem", [key]);
		},
		...
		clear: function(){
			exec(null, null, "Preferences", "clear", []);
		}
	};
