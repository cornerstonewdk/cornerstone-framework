...
;(function (root, doc, factory) {
    ...
    Spinner.prototype.setState = function (state, option2) {
        var d = "disabled"
            , $el = this.$element
            , data = $el.data()
            , val = $el.is("input") ? "val" : "html";
...
//        console.log("toggle");
    ...
    Spinner.prototype.show = function () {
        this.showText();
        $(this.options.spinnerID).show();
    };

    Spinner.prototype.showAll = function () {
        this.showText();
        $(this.options.spinnerID).addClass("all").show();
    };

    Spinner.prototype.showText = function (option) {
        var text = option.loadingText;

        if(typeof text !== "string" && $.trim(text).length < 1) {
            return false;
        }

        $(this.options.spinnerID).find("div.spinner-center").attr({
            "data-content": text
        });
    };
...
//    };
    ...
    $.fn.spinner = function (option, option2) {
        return this.each(function () {
            var $this = $(this)
            ...
            if (option == "show") {
                ...
                data.showAll(option2);
            } else if (option == "hide") {
                data.hide(option2);
            } else if (option) {
                data.setState(option, option2)
            }
        })
    };
