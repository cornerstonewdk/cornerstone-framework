...
function cf_makeDealerSub(obj_subAgencyId, divTargetId)
{
    ...
    $('#'+divTargetId).append('<label class="control-label" for="dealer_sub">실판매처 :</label>\
                             <div class="controls">\
 ...
 */
function cf_makeComponent(frmElementId, type, subagencyNm,copy_target){
    ...
    if(type == 'num_subagency')
    {
        ...
        __getDynamicSubOption(frmElementId,copy_target);
...
function __getDynamicSubOption(frmElementId,copy_target)
{
    var cnt = $(frmElementId + ' #num_subagency').val();
    
    if(copy_target==undefined) copy_target="#id_subagency";
    else copy_target="#"+copy_target;
    var id_subagency_children=$(frmElementId+' '+copy_target).html();
...
//        sndObj.ASYNC        = true;
//        cf_getOptionData(sndObj);
        $(frmElementId + " #tkey_subagency_idx" + i).html(id_subagency_children);
    }
}
...
function cf_initOptionData(functionType, printElement, async, arrSndData, callBackFn, isLovMode){
    ...
    if(isLovMode != undefined) sndObj.IS_LOV_MODE =  isLovMode;
    cf_getOptionData(sndObj);
}
