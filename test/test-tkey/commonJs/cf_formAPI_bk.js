...
FormAPI = function(_pageMode,_formView ,_regModel,_regView) {
    ...
    this.setFormView = function(objData){
        console.error('-> formMapper 변경');
        //if(m_isForm == false) return null;
    ...
    this.chkOldAuth = function(_oldArrAttr, _isBack){
        ...
        if(retValue == false){
            ...
            var ErrObj=new Object();
                ErrObj.MODE = "dialog";
                ErrObj.errMsg="권한이 없습니다.";

            if(_isBack != undefined && _isBack == false){
                ErrObj.historyGo=-1;
    ...
    var __getStrElementId =function(){
        ...
        if(cntArgs == 1){
            ...
            //retValue = __getFormString() + ' ' +  arrElementId.join(",");
    ...
    var __getStrElementNm =function(){
        ...
        if(cntArgs == 1){
            //retValue = __getFormString() + ' ' + ':input[name=' + arguments +']';
    ...
    this.disabled = function(){
        ...
        if(elements instanceof Array){
            for(var i=0, cnt=elements.length; i<cnt; i++){
                elements[i].prop("disabled", isDisabled);

                var dtpicker = elements[i].data('dtpicker');
    ...
    var __setValue = function(_element, _value){
        ...
        default:
            console.error('[DEV] %s 지정한 테그일 경우 추가 작업이 필요할 수 있습니다..', tagName);
            break;
        }
    }
...
//            });
            if(_args[i][3] == undefined || _args[i][3] == false){
                ...
                $(document).on(_args[i][1], __querySelectorStr(_args[i][0]), _args[i][2]);
            }
    ...
    var m_arrOptionData = null; 
    /**
     * 공통 SelectBox 데이터 설정하기
     * @param _arrOptData 공통설정 배열
     */
    this.initOptionData = function(_arrOptData, isLovMode){
        ...
        if(isLovMode == undefined || isLovMode == false){
        	...
        	m_arrOptionData = {};    	        	
        	__seLovEventHandler(_arrOptData);
        	__drawDivSelectBox();
        	
        }        
    }
    
    var __drawDivSelectBox = function(){
        var divScreen    = "#"+$("div[data-page=Y].current").attr("id");    
        var divSelectBox = "<div id='divSelectBox' data-page='Y'></div>";
        
        $(divScreen).parent().prepend(divSelectBox);
    }
    
    var __seLovEventHandler = function(_arrOptData){
    	
    	for(var i=0, cnt = _arrOptData.length; i < cnt; i++){

    		var elements = __querySelector(_arrOptData[i][1]);
    		
    		for(var j=0, cntElement = elements.length; j < cntElement; j++){
    		
    			_arrOptData[i][1] = __querySelectorStr(_arrOptData[i][1])
    			m_arrOptionData[elements[j].name] = _arrOptData[i];
    		}    		

    		elements.on('click', function(event){
    			
    			var elementNm = event.target.name;
            	var optData   = m_arrOptionData[elementNm];
            	
                var callBackFn = function(){
                	
                	var divScreen = "#"+$("div[data-page=Y].current").attr("id");
                	
                	cf_setPageParam(optData);
                    transScreen.cf_transScreen(false, '#divSelectBox', divScreen, select_box.df_init);                  
                };
                
                cf_loadPage("/quicksearch/select_box", '#divSelectBox', '#divBody', callBackFn);
                
            	//cf_initOptionData(optData[0], __querySelectorStr(optData[1]), optData[2],  optData[3], optData[4], true);
            });
    	}    	
    }
