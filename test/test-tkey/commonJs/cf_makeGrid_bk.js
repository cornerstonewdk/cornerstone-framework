...
MakeGrid = function(_objDef, _colInfo, _gestureView) {
    ...
    var m_limitItem      = 50;                   //한페이지에 표시할 항목수
    ...
    var __movePage = function(_pageNum){
        ...
        //if(m_curPage == _pageNum && (m_pagingType != 'NUMBER' && m_pagingType != 'AN-SCROLL')) return;

        //console.log('__movePage : %s', _pageNum);
        //if(m_curPage == _pageNum) return;
        m_curPage = _pageNum;
    ...
    //    console.log('__cbDrawList');
        ...
        var thead = '';
        var table = '';
        if(m_pagingType == 'NUMBER' || m_pagingType == 'GESTURE'){
...
//        });
        ...
        $(document).on('click', buttonArea, function(event){            
    ...
    var __setTrEventHandler = function(_rowClickFunc){
        ...
        var selector = m_divList + " " + "#" + m_tableID + " " + "tr[id^=row_idx]";
    ...
    var __setAllCheckEventHandler = function(){
        ...
        $(document).on("click", allCheckSelector, function(event){
alert('all');
            event.preventDefault();
        ...
        case 'indexnum':
            ...
            if(type == 'indexnum'){
                ...
                if(tooltip != undefined){
                    /*동적으로는 안 먹히네..
                    var toggle = $(m_divList).data('toggle');
                    if(toggle == undefined){
                        $(m_divList).attr('data-toggle', 'tooltip');
                    }
                    */
                    if(_value == ''){
                        retValue += '<a rel="tooltip" data-placement="right" title="' + _value + '"></a>';
                    }else{
                        retValue += '<a rel="tooltip" data-placement="right" title="' + _value + '">Here</a>';    
...
//    var __drawModalArea = function(){
//
//        //var selector = m_divList + ' ' + '#divGridModalArea';
//
//        if($(m_divList).closest("[data-page=Y]").find('#divGridModalArea').length != 0) return;
//
//        var html = '';
//        html += '<div id="divGridModalArea">';
//        html += '</div>';
//
//        $(m_divList).closest("[data-page=Y]").append(html);
//    }
...
//        }
    ...
    this.setTrColor = function(_color, _trElement){
        ...
        var cnt = $(backColorSelector, _trElement).length;

        if(cnt == 0){//TD에 배경색상이없을 경우
            $(_trElement).css("background-color", _color);
        }else{       //TD에 배경색상이 존재할 경우 존재하는 TD를 제외한 나머지 적용
            $(_trElement).not(backColorSelector).css("background-color", _color);
        }
    }
