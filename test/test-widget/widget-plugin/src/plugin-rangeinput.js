    ...
    if (navigator.userAgent.match("Android.*/4.0 Mobile Safari/")) {
        $("input[type=range]").each(function (i) {
            ...
            if (typeof options === "object" && options.inputShow) {
                var $rangeTarget = $(this), $numTarget = $("<input type='tel' value='" + $rangeTarget.val() + "' class='widget-range btn-block figure range' />");
            ...
            $(this).on('touchstart.inputrange',function (e) {
            }).on("touchmove.inputrange",function (e) {
                }).on("touchend.inputrange", function (e) {
                });
        });
    } else {
        $("input[type=range]").each(function (i) {
