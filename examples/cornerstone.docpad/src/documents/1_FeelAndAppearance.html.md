---
title: "Reference: 1. Feel And Appearance"
date: "2012-10-03"
layout: "post"
tags: 'post'
outline: "이 문서는 구현을 위한 설계문서로써 제품의 모양새와 느낌을 정의하고 설명함"
---
...
### 구현방안
방향과 개념에 따른 구체적인 방안으로 [트위터 부트스트랩][BtS]을 기본으로하여 [부트스트랩의 사용법][BtS:설명]을 그대로 이어받아 부트스트랩의 장점을 잘 활용할 수 있도록 함. 구체적으로는 [부트스트랩 확장][Bts:확장]의 방식대로 [Less](http://lesscss.org/)를 통해 부트스트랩을 확장한다.
...
- 코드관리
	- **git을 통해 코드 관리**
	- BtS와 같은수준의 별개 폴더를 만들어 less를 관리
		- Mobile Booted : BtS에 less를 추가만 함
		- Wireframe : 가급적 BtS의 변수저장 less만 변경 및 less를 추가만 함
		- Cornor stone : BtS의 less를 확장
...
- 디자이너
	...
	- 별도의 화면설계가 없는경우 사용성을 고려하여 BtS을 기준으로 디자인
...
- 퍼블리셔
	...
	- [부트스트랩에서 권장하는 확장][Bts:확장]방식으로 BtS을 확장하여 구현 
	- 변수, 모듈화등 BtS에서 사용하는 방식대로 디자인을 LESS로 스타일링
	- 작업의 우선순위는 Bootstrap 테마로 디자인을 기다리지 않고 기존 BtS에 제공되는 디자일은 재사용하여 구현
	- 스타일링 관련 가이드 문서 작성 
- 개발자
	- 제품의 기능을 구현
	- 특별히 성능이슈와 관련된 모든 영역에 오너쉽을 가지고 지속적으로 관리
	- 저장소, 테스트서버에 오너쉽을 가지고 지속적으로 관리
	- 퍼블리셔가 BtS의 빌드체계를 활용할 수 있도록 저장소 구성
	- 별도의 구현이 필요할 시 최대한 BtS의 플러그인 작성 방식을 따름 
...
#### 테마
제공되는 느낌과 모양세의 커다란 범주를 테마라고 하며 3가지를 테마를 제공하며 BtS의 특성인 [BtS > LESS](http://twitter.github.com/bootstrap/less.html)로 작성되어 [BtS > Customize](http://twitter.github.com/bootstrap/download.html)와 같이 사용자화 가능함으로 개별 테마 안에서 BtS에 제공된 LESS의 변수를 조절함으로써 제공되는 영역에서 손쉽게 사용자화된 테마 생성이 가능함. 따라서 테마를 구현시 기본적으로 BtS에 없는 요소들도 BtS과 마찬가지로 LESS로 작성되어 적절히 변수가 제공되어 BtS의 방식대로 사용자화가 가능해야 함
...
기본적으로 부트스트랩과 마찬가지로 [normalize.css][normalize]를 사용하여 스타일의 기반을 만듬
	...
	1. 글꼴지원  
	BtS에서는 영문폰트에 대한 것만 정의된 상태로 코너스톤은 지원단말에 내장된 한글폰트에 대한 것들이 고려되어야함
...
#### [폼][Bts > BaseCSS > Forms]
위젯. BtS의 BaseCSS에 해당하나 본 사업에서는 별도의 항목으로 분리토록함. 기본적인 개요는 본 문서의 `베이스`를 참고 하도록 함. 아래의 구현, 기능, 느낌과 모양새에 관련한 방안과 범위는 테마중 Conor Stone을 기준으로 함
...
- 구현 : BtS에 있는것을 스타일 확장 **Vertical button group은 2.1.0에 신규 추가 되었음**
- 기능 : 기능없음
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용. 
...
##### 폼스타일 구현
BtS은 기본적으로 폼에 스타일을 별도로 구현하지 않으며, 웹 표준에서 권장되는 기능명세로는 실질적으로 폼에 스타일 구현이 어려움. Cornerstone에서는 모바일 친화적, 제품 특징 구현을 위해 폼에 스타일을 적용하는것이 필요하며 이를 위해 자바스크립트를 통해 별도의DOM을 삽입하는 방식을 취하되 최소한의 범위로 한정함. 각 폼의 DOM삽입체계는 유사하고 규칙성이 있으며 동시에 가이드 문서상에 적절히 제시되어 개발자가 이해하고 사용하기 쉽도록함
...
- 구현 : BtS에 있는것을 스타일 확장 
- 기능 : BtS 기준. 스크롤뷰 플러그인과 연동가능하도록 함
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용
...
- 구현 : BtS에 있는것을 스타일 확장 
- 기능 : BtS 기준. 모바일에서는 hover가 아니라 터치시 작동함
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용
...
- 구현 : BtS에 있는것을 스타일 확장 
- 기능 : BtS 기준. 터치에 맞추어 작동 하도록 함
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용

##### [carousel](http://twitter.github.com/bootstrap/javascript.html#carousel)
개발자는 필요에 따라서 BtS제공 플러그인이 아닌 다른 플러그인을 사용하여 구현

- 구현 : BtS에 있는것을 스타일 확장 
- 기능 : BtS 기준. 좌우 끌어당기는 터치동작을 통해 그림 변경 가능
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용
...
##### list view 
BtS에는 없는 모바일에서 사용하는 터치 친화적인 스크롤가능한 화면을 만들어주는 기능성 위젯
...
- 구현 : **BtS에 없으며 새로이 만들어야 함**
	- Tiny MCE jQuery 버전을 사용하여 기능을 차용하고 테마스톤/BtS에 맞추어 적절하게 테마를 제공
