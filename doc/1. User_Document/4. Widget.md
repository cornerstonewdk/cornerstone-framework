...
```
<!-- data-featurd="datatable"를 정의하므로 데이터테이블을 적용한다. -->
<table class="table table-striped" data-featurd="datatable">
    <!-- 머리글 정의 -->
    <thead>
        <tr>
...
### 미디어엘리먼트
 ...
 미디어엘리먼트는 기본적으로 HTML5 video와  audio를 제공하지만 지원하지 않는 브라우저 또한 fallback을 통해 동일한 HTML/CSS로 같은 결과를 얻을 수 있는 장점이 있다.

![<미디어 지원표>](<http://azamara.io/img/media-support.png>)

출처) mediaelement.js 홈페이지

용어설명 
- H.264 : 매우 높은 데이터 압축률을 가지는 디지털 비디오 코덱 표준으로 MPEG-4 파트 10 또는 AVC(Advanced Video Coding)라 부르기도 한다.
- OGG :컴퓨터의 멀티미디어 파일 포맷
- WebM : 로열티 비용이 없는 개방형 고화질 영상 압축 형식의 비디오 포맷이며 HTML5 동영상과 함께 이용한다. 2010년 5월 19일에 처음 나왔으며이 프로젝트는 구글의 후원을 받아 개발된다.
