...
위젯 폼은 4가지 폼 레이아웃을 제공한다. 서로 다른 타입의 폼 레이아웃은 마크업 수정을 필요로 한다.
하지만 컨트롤 그 자체는 같은 성질을 가진채 남아 있게 된다.

이름 | 클래스 | 설명
:-- | :-- | :--
Vertical(수직) | .form-vertical(기본) | 라벨과 입력요소를 수직으로 정렬한 스타일
Inline(일직선상) | .form-inline  | 폼 요소를 수평 일직선상으로 정렬하는 스타일
Search(검색) | .form-search | 검색 스타일을 위한 좌우가 둥근 입력 박스 스타일
Horizontal | .form-horizontal | 라벨과 입력요소만 수평으로 정렬한 스타일
...
## TeNew Business Process Modelling Shapes plus New Diagram Videos & Improved Helpxtarea
