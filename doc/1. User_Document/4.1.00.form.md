...
input박스 전후로 텍스트나 버튼을 추가할수 있다.

__텍스트 대신 버튼__

라벨을 사용하지 않고 동작하지 않는 버튼위에 텍스트를 넣음으로 써 가시효과를 높일때 사용한다.

``` cm, { iframe-height : 359px }
	<form>
...
입력상자에 들어있는 값을 수정할 수 없는 형태. 마우스 오버 시 커서가 에디팅 불가능 형태 커서로 바뀐다.
