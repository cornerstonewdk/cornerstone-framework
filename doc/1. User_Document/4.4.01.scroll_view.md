...
터치환경에서 네이티브적인 느낌을 주는 ScrollView는 안드로이드에서는  기본적으로 제공을 하지 않으며, iOS 계열은 CSS를 통한 제공을 하지만 아이템이 많은 경우 성능 저하 문제가 있다. 코너스톤에서는 성능 및 몇가지 기능 등을
...
``` cm,{"iframe-no-padding" : true, "iframe-height": "300px", "iframe-auto-height": false}
...
``` cm,{"iframe-no-padding" : true, "iframe-height": "600px", "iframe-auto-height": false }
