...
touchcancel | 화면에 손이 닿은 상태에서 전화가 오거나 하는 등의 일이 발생하면 시스템에 의해 터치가 취소되면서 발생한다. 일부 기기에서는 화면에서 손을 떼는 경우에 발생할 수도 있다.
...
속성명 | 기본값 | 설명
:---- | :---- | :----
prevent_default | false | true로 지정하면 브라우저의 기본 동작을 막는다. 예를 들어 드래그를 할 때 화면의 스크롤을 막고, 손가락을 오므리거나 벌릴 때 줌 인/아웃이 되지 않도록 막는다.
css_hacks | true | userSelect, touchCallout, userDrag, tapHightlightColor css를 추가한다.
drag_vertical | true | 세로로 드래그할 수 있게 한다.
drag_horizontal | true | 가로로 드래그할 수 있게 한다.
drag_min_distance | 20 | 픽셀 단위. 최소한 이 픽셀만큼 움직여야 drag 동작으로 인식한다.
swipe_time | 200 | ms 단위. 이 시간 이내로 움직여야 swipe 동작으로 인식한다.
swipe_min_distance | 20 | 픽셀 단위. 최소한 이 픽셀만큼 움직여야 swipe 동작으로 인식한다.
scale_treshold | 0.1 | scale이 최소한 이 값만큼 변화해야 transform 동작으로 인식한다.
rotation_treshold | 15 | 도 단위. rotation이 최소한 이 값만큼 변화해야 transform 동작으로 인식한다.
tap_max_interval | 300 | ms 단위. tap 이후 이 시간 이내에 다시 tap 하면 doubletap으로 인식된다.
tap_double_distance | 20 | 픽셀 단위. 두번의 tap이 이 거리 내에 있어야 doubletap으로 인식된다.
hold_timeout | 500 | ms 단위. 이 시간 이상 터치가 유지되어야 hold로 인식된다.
