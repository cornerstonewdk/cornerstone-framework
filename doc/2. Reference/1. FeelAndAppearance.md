...
- 공통
	...
	- **기획자를 포함해 4자협업이 필요함으로 이슈가 생기면 지체없이 [저장소관리 사이트](http://prandy.iptime.org:3000)에 이슈 등록을 하여 기록으로 남는 의사결정 진행**
	- 필요하면 누구나 본 문서를 편집하고 변경토록 함
- 코드관리
	- **git을 통해 코드 관리**
	- BtS와 같은수준의 별개 폴더를 만들어 less를 관리
		- Booted : BtS에 less를 추가만 함
...
- 퍼블리셔
	...
	- Cornor Stone의 기본개념인 HTML(내용)/CSS(모양)/JS(기능)에 맞추어 CSS를 디자인 구현에 극대화하여 최대한 사용
...
- 구현 : BtS에 있는것을 스타일 확장 
- 기능 : 기능없음
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용. 기본적으로 본 문서의 `button`과 다를 바 없음
...
BtS에서는 가로형 그룹만 제공되나 Cornor Stone의 폼에서 제공하는 것과 같이 세로형 그룹과 연계되어 사용이 가능함
...
##### `?` [sketch pad canvas](http://intridea.github.com/sketch.js/)
캔버스 기반 터치 스케치 패드 
...
- 기능 : 화면에 고정되어 터치로 끌어서 내용을 읽음 
	- fixed bar, scroll view 와 함께 유연하게 작동 
	- 안쪽에는 간단한 DOM이 표현되며 기능이 들어간 내용이 들어갈시 터치 작동을 방해할 수 있음으로 이에대한 적절한 가이드가 필요함 
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용 _기능만 있는 위젯으로 디자이너가 특별히 다룰것이 없음_

###### `utility?` fixed bar
BtS에는 없는 모바일에 사용이 적합한 상/하단의 고정 바를 만들어 주는 기능성 위젯

- 구현 : **BtS에 없으며 새로이 만들어야 함**
- 기능 : 상단 하단으로 고정된 바. 콤포넌트나 플러그인의 다른 위젯들과 함께 사용가능
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용

- 참고사항
	- [jQM > Toolbar > Header](http://jquerymobile.com/demos/1.2.0-alpha.1/docs/toolbars/docs-headers.html), [jQM > Toolbar > Footer](http://jquerymobile.com/demos/1.2.0-alpha.1/docs/toolbars/docs-footers.html)
...
#### `?`[colorpicker](http://www.eyecon.ro/colorpicker-and-datepicker-for-twitter-bootstrap.htm)
...
- 기능 : 참고의 제3자 jQuery플러그인을 기준으로 함
	...
	- scroll view를 상속받아 사용됨
	- 플러그인의 scroll view, fixed bar와 유연하게 작동함
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용
	- jQM > list의 상당수를 제공

- 참고사항
	- [jQM > lists](http://jquerymobile.com/demos/1.2.0-alpha.1/docs/lists/index.html)
...
- 구현 : **BtS에 없으며 새로이 만들어야 함**
- 기능 : 링크의 제3자 jQuery플러그인을 기준으로 함 
	- 모바일상황에서는 위지윅 에디터 기능이 없으며 최대한 단순하게 기능제한
- 느낌과 모양새 : 방향성에 맞추어 모바일에 알맞게 적용
