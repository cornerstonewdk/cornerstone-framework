...
1. [레이아웃](http://twitter.github.com/bootstrap/scaffolding.html#layouts)  
...
_현제 모바일에서 어떻게 해야될지 방안이 없음_  
_현제 투트랙 전략에 어떻게 적용되는지 방안이 없음_  
_투트랙에서 반응형 페이지와 반응형 레이아웃으로 나누는건 어떨까?_  

#### [폼][Bts:BaseCSS:Forms]
부트스트랩을 따르되 모바일에 한하여 아래와 같이함

- 인라인 폼 클래스 `.form-inline`
	- 폰 : 인라인 폼 클래스가 작동하지 않고 한열당 한개의 폼만 있도록 함  
	필요하다면 `.form-inline-mobile`를 삽입하여 별도로 구현하고 주로 label용으로만 사용할 것을 권장
	- 타블렛 : 인라인 폼 클래스 사용 
